    ...
    public static class FunctionMetadataExtensions
    {
        ...
        public static async Task<FunctionMetadataResponse> ToFunctionMetadataResponse(this FunctionMetadata functionMetadata, HttpRequest request, ScriptHostConfiguration config)
        {
            ...
            var response = new FunctionMetadataResponse
            {
                ...
                ScriptHref = VirtualFileSystemBase.FilePathToVfsUri(Path.Combine(config.RootScriptPath, functionMetadata.ScriptFile), baseUrl, config),
                ConfigHref = VirtualFileSystemBase.FilePathToVfsUri(functionMetadataFilePath, baseUrl, config),
                ScriptRootPathHref = VirtualFileSystemBase.FilePathToVfsUri(functionPath, baseUrl, config, isDirectory: true),
                TestDataHref = VirtualFileSystemBase.FilePathToVfsUri(functionMetadata.GetTestDataFilePath(config), baseUrl, config),
